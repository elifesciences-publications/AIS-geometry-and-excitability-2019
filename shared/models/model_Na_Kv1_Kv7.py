...
In addition, the distal half of the AIS contains Kv7-like channels.

'''

from brian2 import *

__all__ = ['model_Na_Kv1_Kv7']

def model_Na_Kv1_Kv7(params,  resting_vm = 75.*mV, Na_start = 5.*um, Na_end = 35.*um, density = False, gna_tot = 350.*nS, gk_tot = 150.*nS, gm_tot = 250.*nS, gna_density = 4000.* (siemens / meter ** 2), morpho = None):
    '''
    ...
    # Equations
    eqs = '''
    Im = (gL*(EL-v) + gNa*m*h*(ENa-v) + gK*n**8*(EK-v) + gM*(EM-v)) : amp/meter**2
    ...
    neuron = SpatialNeuron(morphology=morpho, model=eqs, Cm=Cm, Ri=Ri,
                                    namespace=dict(gL=gL, EL=EL, ENa=ENa, EK=EK, EM = EM,
    ...
    if Na_start == Na_end:
        initial_segment = morpho.axon[Na_end]
        if density == False:
            neuron.gNa[initial_segment] = Gna/neuron.area[initial_segment]
            neuron.gK[initial_segment] = Gk/neuron.area[initial_segment]
            neuron.gM[initial_segment] = Gm/neuron.area[initial_segment]
            #print neuron.gNa[initial_segment], neuron.gK[initial_segment], neuron.gM[initial_segment]
        else:
            neuron.gNa[initial_segment] = gna_density
            neuron.gK[initial_segment] = params.gk_dens
            neuron.gM[initial_segment] = params.gm_dens
    else:
        initial_segment = morpho.axon[Na_start:Na_end]
        distal_ais = morpho.axon[Na_start+AIS_length/2:Na_end]
        #print Na_start, AIS_length, Na_end, Na_start+AIS_length/2
        if density == False:
            ...
            neuron.gM[distal_ais] =  Gm/(pi*(AIS_length/2)*morpho.axon.diameter[0])
            #print neuron.gNa[initial_segment], neuron.gK[initial_segment], neuron.gM[initial_segment]
        else:
            neuron.gNa[initial_segment] = gna_density
            neuron.gK[initial_segment] = params.gk_dens
            neuron.gM[distal_ais] = params.gm_dens
