...
'''
Biophysical model of an AP in an axo-dendritic neuron. 
...
__all__ = ['model_Na_Kv1_axodendritic']

def model_Na_Kv1_axodendritic(params,  resting_vm = 75.*mV, Na_start = 5.*um, Na_end = 35.*um, \
    ...
    if morpho is None:
        ...
        dend_length = 1000.*um 
        soma_diameter = 30. * um
        stem_length = 7.*um # Thome 2014
        stem_diam = 2.*um # Thome 2014
        morpho = Soma(diameter=soma_diameter)
        dendrite = Cylinder(diameter=dend_diam, length=dend_length, n=500)
        stem = Cylinder(diameter=stem_diam, length=stem_length, n=7)
        morpho.dendrite = dendrite
        morpho.stem = stem
        # branches
        # axon branches
        branch_diam = stem_diam/(2**(2./3))
        morpho.stem.dendrite = Cylinder(diameter=branch_diam, length=1000*um, n=1000)
        morpho.stem.axon = Cylinder(diameter=branch_diam, length=1000*um, n=1000)
    ...
    if Na_start == Na_end: # point AIS
        initial_segment = morpho.stem.axon[Na_start]
        if density == False:
        ...
        initial_segment = morpho.stem.axon[Na_start:Na_end] 
        if density == False:
            neuron.gNa[initial_segment] =  Gna/(pi*AIS_length*morpho.stem.axon.diameter[0])
            neuron.gK[initial_segment] =  Gk/(pi*AIS_length*morpho.stem.axon.diameter[0])
        else:
            neuron.gNa[initial_segment] = gna_density
            neuron.gK[initial_segment] = gk_density
