...
The axon beyond the AIS is myelinated.
...
def model_Na_Kv1_myelin(params,  resting_vm = 75.*mV, Na_start = 5.*um, Na_end = 35.*um, \
    ...
    ### Passive parameters myelin
    Cm_myelin = params.Cm/4
    gL_myelin = params.gL/4
    ...
    Cm_neuron = hstack([Cm/(uF / cm ** 2) * ones(morpho.dendrite.n + 1 + int(Na_end/um)), Cm_myelin/(uF / cm ** 2) * ones(morpho.axon.n - int(Na_end/um))]) * uF / cm ** 2
    ### AIS
    ...
    neuron = SpatialNeuron(morphology=morpho, model=eqs, Cm=Cm_neuron, Ri=Ri,
    ...
    if Na_start == Na_end: # point AIS
        ...
        distal_axon = morpho.axon[Na_start:]
        if density == False:
        ...
        distal_axon = morpho.axon[Na_end:]
        if density == False:
    ...
    ## Myelin
    neuron.gL[distal_axon] = gL_myelin
    
    # Initialisation
    neuron.v = EL
    neuron.h = 1
    
    #print ('Time cst:', neuron.time_constant[0])
